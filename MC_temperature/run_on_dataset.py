...
def main(
    ...
    mean_std_probs = [r.get("mean_std_prob") for r in per_image_results if r.get("mean_std_prob") is not None]
    std_std_probs = [r.get("std_std_prob") for r in per_image_results if r.get("std_std_prob") is not None]
    ...
    for name in class_names:
        ...
        if not (accs_c or ents_c or stdps_c):
            continue
        per_class[name] = {
            "pixel_accuracy": {
